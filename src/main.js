...
'use strict';
var LOCAL_SAVE_KEY = 'KuBiTionAdvanture.localSave.v1';
var LOCAL_SAVE_VERSION = 1;
// var move=function(e){
...
var NormalMenuComponent = React.createClass({
    contextTypes:{
        ...
        exportSaveData      :React.PropTypes.func.isRequired,
        importSaveFile      :React.PropTypes.func.isRequired,
    ...
    exportSaveData:function() {
        this.context.exportSaveData();
    },
    importSaveData:function(sender) {
        var input = sender.nativeEvent.srcElement ? sender.nativeEvent.srcElement : sender.nativeEvent.target;
        var file = input.files && input.files[0];
        if(file){
            this.context.importSaveFile(file);
        }
        input.value = '';
    },
    openImportFile:function() {
        this.refs.importSaveInput.click();
    },
    quitMenu:function(){
                ...
                return (
                    <div className = 'skillMenu'>
                        <div style = {{marginTop:10}}>
                            <div>
                                <p style = {{color:'#ddd'}}>网页存档保存在当前浏览器中，建议定期导出备份。</p>
                            </div>
                            <div>
                                ...
                                <BtnComponent handleClick = {this.exportSaveData}>导出</BtnComponent>
                                <BtnComponent handleClick = {this.openImportFile}>导入</BtnComponent>
                                <input ref = "importSaveInput" onChange = {this.importSaveData} type = "file" accept = ".json,application/json" style = {{display:'none'}} />
                            </div>
                        </div>
...
var MainComponent = React.createClass({
    ...
    getChildContext: function() {
        return {
            ...
            exportSaveData      : this.exportSaveData,
            importSaveFile      : this.importSaveFile,
    ...
    loadState:function(data){
        ...
        this.setState({currentScene:'home'});
        var level = 0;
        for(var science in data.boxSaveData['scienceTable'].things){
            if(ITEM_DATA[science].type == 'bagSizeBonus'){
                level ++;
            }
        }
        var boxSaveData = data.boxSaveData;
        boxSaveData['bag'].size = BAG_BASE_SIZE + level;
        this.setState({boxSaveData:boxSaveData});
    },
    makeSaveData:function(){
        ...
        return {
            ...
            campSaveData:clone(this.state.campSaveData),
            coolDownSaveData:clone(this.state.coolDownSaveData),
            currentEquip:clone(this.state.currentEquip),
            currentScene:this.state.currentScene,
            defaultWeapon:clone(this.state.defaultWeapon),
            dungeonSaveData:clone(this.state.dungeonSaveData),
            durableSaveData:clone(this.state.durableSaveData),
            eventSaveData:clone(this.state.eventSaveData),
            generation:this.state.generation,
            isDueling:false,
            maouLevel:this.state.maouLevel,
            mstState:{},
            placeSaveData:clone(this.state.placeSaveData),
            ...
            tradeSaveData:clone(this.state.tradeSaveData),
        };
    },
    makeSavePayload:function(){
        return {
            version:LOCAL_SAVE_VERSION,
            savedAt:(new Date()).toISOString(),
            data:this.makeSaveData(),
        };
    },
    parseSavePayload:function(str){
        var payload = JSON.parse(str);
        if(payload && payload.version == LOCAL_SAVE_VERSION && payload.data){
            return payload.data;
        }
    ...
    loadData:function (str) {
        var data;
        try{
            data = this.parseSavePayload(str);
        }catch(e){
            alert('存档损坏，无法读取。');
            return false;
        }
    ...
    download:function(){
        var str = null;
        try{
            str = localStorage.getItem(LOCAL_SAVE_KEY);
        }catch(e){
            alert('无法访问浏览器本地存储。');
            return;
        }
    ...
    exportSaveData:function(){
        ...
        link.href = url;
        link.download = 'kubition-save-' + payload.savedAt.replace(/[:.]/g,'-') + '.json';
    ...
    importSaveFile:function(file){
        ...
        reader.onload = function(event){
            ...
            try{
                this.parseSavePayload(text);
                localStorage.setItem(LOCAL_SAVE_KEY,text);
            }catch(e){
                alert('导入失败，存档文件无效。');
                return;
            }
            alert('导入成功，可以点击读取恢复存档。');
        }.bind(this);
